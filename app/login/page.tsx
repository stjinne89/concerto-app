...
import { useState, Suspense, useEffect } from 'react' // <-- Hier is 'useEffect' toegevoegd!
import { useSearchParams } from 'next/navigation'
import { Loader2, AlertCircle } from 'lucide-react'

// Klein hulpcomponentje om errors te lezen uit de URL
...
export default function LoginPage() {
  ...
  // Hier definiëren we alle 'state' variabelen
  const [isLoginMode, setIsLoginMode] = useState(true)
  const [loading, setLoading] = useState(false) // <-- Deze miste, is nu toegevoegd!

  // Dit effect kijkt of er een error in de URL staat en schakelt naar 'Registreren' indien nodig
  useEffect(() => {
    const error = searchParams.get('error')
